...
module MultihopOscilloscopeC @safe(){
  uses {
	...
	interface SplitControl as TestRadioControl;
...
implementation {
  ...
  */
  ...
  event void TestRadioControl.startDone(error_t err) {
    if (err == SUCCESS) {
     dbg("MultihopOscilloscopeC", "TestRadioControl was run successfully @ %s.\n", sim_time_string());
    }
  ...
  {
  if (err == SUCCESS) {
  dbg("MultihopOscilloscopeC", "TestRadioControl was stop successfully @ %s.\n", sim_time_string());
  }  
  }
