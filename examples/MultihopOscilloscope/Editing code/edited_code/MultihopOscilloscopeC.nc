...
module MultihopOscilloscopeC @safe(){
  uses {
	...
	//edited code
	interface Timer<TMilli> as EditedTimer;
	interface SplitControl as RadioControl;
...
implementation {
  ...
  */
  ...
  static void report_received() { call Leds.led2Toggle(); }
   ...
   //edited code
  event void EditedTimer.fired() {
    testCounter++;
	dbg("MultihopOscilloscopeC", "EditedTimer was run successfully @ %s.\n", sim_time_string());
  }
  ...
  dbg("MultihopOscilloscopeC", "RadioControl was stop successfully @ %s.\n", sim_time_string());
	...
	if (call EditedTimer.isRunning() != TRUE)
	{
		...
		dbg("MultihopOscilloscopeC", "EditedTimer still running @ %s.\n", sim_time_string());
		}
	else
	{
	    post testTask();
		dbg("MultihopOscilloscopeC", "EditedTimer stop running @ %s.\n", sim_time_string());
		}
