	...
	uses interface Timer<TMilli> as EditeTestdTimer;
	uses interface SplitControl as EditedTestRadioControl;
	...
	implementation {
	  ...
	  /* task content */
	   	sharedCounter++;
		if (call EditeTestdTimer.isRunning() != TRUE)
		{
			post incrementCounter();
			dbg("PacketParrotP", "EditeTestdTimer still running @ %s.\n", sim_time_string());
			}
		else
		{
			dbg("PacketParrotP", "EditeTestdTimer stop running @ %s.\n", sim_time_string());
			}
	 }
	   ...
	   /* event content */
	    ...
	    event void EditeTestdTimer.fired() {
	   /* event content */
	       sharedCounter++;
		dbg("PacketParrotP", "TestTimer was run successfully @ %s.\n", sim_time_string());
	 }
	    event void EditedTestRadioControl.startDone(error_t err) {
	   /* event content */
	   if (err =! SUCCESS) {
	   call EditedTestRadioControl.stop();
	     
	    }
		dbg("PacketParrotP", "EditedTestRadioControl was run successfully @ %s.\n", sim_time_string());
	 }
	    event void EditedTestRadioControl.stopDone(error_t err) {
	   /* event content */
	   if (err =! SUCCESS) {
	  dbg("PacketParrotP", "EditedTestRadioControl was stop not successfully @ %s.\n", sim_time_string());
	  }  
	 }
